...
static constexpr const DWORD g_dwDirectoryOffset = 1+(MAX_PATH-12)+1;        //12 = "direct3d8\Kernel.dll`" - "Main.pro`"
static constexpr const DWORD g_dwPathMargin = 22;        //"\direct3d8\Kernel.dll`"
    ...
    if (dwLen >= 4 && dwLen < MAX_PATH)
    {
        ...
        if (pDelim >= pPath+2 && pDelim <= pPath+MAX_PATH-g_dwPathMargin)
        {
            ...
            *++pDelim = L'\0';
            ...
            FCopyMemoryW(pDirectory+(pDelim-pPath), L"direct3d8\\");
