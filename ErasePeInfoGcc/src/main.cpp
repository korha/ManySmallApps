...
static constexpr const DWORD g_dwSizeMax = 8*1024*1024;
                        ...
                        if (BYTE *const pBtFileMap = static_cast<BYTE*>(MapViewOfFile(hFileMapping, FILE_MAP_WRITE, 0, 0, 0)))
                        {
                            ...
                            if (pImgDosHeader->e_magic == IMAGE_DOS_SIGNATURE && pImgDosHeader->e_lfanew == g_lFaNew)
                            {
                                ...
                                if (pImgNtHeaders->Signature == IMAGE_NT_SIGNATURE)
                                {
                                    ...
                                    {
                                        IMAGE_SECTION_HEADER *pImgSectionHeaderIt = pointer_cast<IMAGE_SECTION_HEADER*>(pointer_cast<BYTE*>(pImgFileHeader) + sizeof(IMAGE_FILE_HEADER) + wSizeOfOptionalHeader);
                                        const IMAGE_SECTION_HEADER *const pImgSectionHeaderEnd = pImgSectionHeaderIt + pImgNtHeaders->FileHeader.NumberOfSections;
                                        if (static_cast<DWORD>(pointer_cast<const BYTE*>(pImgSectionHeaderEnd)-pBtFileMap) < iFileSize.LowPart)
                                        {
                                            for (; pImgSectionHeaderIt < pImgSectionHeaderEnd; ++pImgSectionHeaderIt)
                                                if (FCompareMemory(pImgSectionHeaderIt->Name, ".rdata\0", IMAGE_SIZEOF_SHORT_NAME))
                                                    break;
                                            if (pImgSectionHeaderIt < pImgSectionHeaderEnd)
                                            {
                                                if (pImgSectionHeaderIt->PointerToRawData < iFileSize.LowPart &&
                                                        pImgSectionHeaderIt->SizeOfRawData > 0 &&
                                                        pImgSectionHeaderIt->SizeOfRawData < iFileSize.LowPart &&
                                                        pImgSectionHeaderIt->SizeOfRawData%512 == 0 &&
                                                        pImgSectionHeaderIt->PointerToRawData + pImgSectionHeaderIt->SizeOfRawData < iFileSize.LowPart)
                                                {
                                                    BYTE *pBtIt = pBtFileMap + pImgSectionHeaderIt->PointerToRawData;
                                                    const BYTE *const pBtEnd = pBtIt + pImgSectionHeaderIt->SizeOfRawData - g_dwApproxMargin;
                                                    wError = L"GCC signature not found. Cancel All?";
                                                    constexpr const char cSignature[] = {'G','C','C',':',' ','('};
                                                    do
                                                    {
                                                        if (FCompareMemory(pBtIt, cSignature, 6))
                                                        {
                                                            FZeroMemory(pBtIt, (pBtEnd + g_dwApproxMargin) - pBtIt);
                                                            wError = nullptr;
                                                            break;
                                                        }
                                                    } while (++pBtIt < pBtEnd);
                                                    ...
                                                    wError = L"Incorrect .rdata section header. Cancel All?";
                                            }
                                            else
                                                wError = L".rdata section not found. Cancel All?";
                                        }
                                        else
                                            wError = L"Invalid Section header. Cancel All?";
                                    }
                                    else
