                ...
                *wCmdLine = L'\0';
                ...
                if (dwArgLen >= 4 && dwArgLen < MAX_PATH)
                {
                    ...
                    if (hFile != INVALID_HANDLE_VALUE)
                    {
                        ...
                        {
                            alignas(unsigned long) BYTE btBuf[(1+MAX_PATH+1 + 1+MAX_PATH+1 + MAX_PATH-10)*sizeof(wchar_t)];        //"idaq64.exe"
                            DWORD dwSize;
                            ...
                            if (dwSize)
                            {
                                        ...
                                        pImgDosHeader->e_lfanew >= 8 && pImgDosHeader->e_lfanew <= 1024 && pImgDosHeader->e_lfanew%sizeof(DWORD) == 0)
                                {
                                    ...
                                    if (pImgNtHeaders->Signature == IMAGE_NT_SIGNATURE)
                                        if (const wchar_t *const wAppend = pImgNtHeaders->FileHeader.Machine == IMAGE_FILE_MACHINE_I386 ? L"idaq.exe\" \"" :
                                                (pImgNtHeaders->FileHeader.Machine == IMAGE_FILE_MACHINE_AMD64 ? L"idaq64.exe\" \"" : nullptr))
                                        {
                                            ...
                                            if (dwSize >= 4 && dwSize < MAX_PATH)
                                            {
                                                ...
                                                if (pDelim >= wBuf+2 && pDelim <= wBuf+MAX_PATH-12)        //"\idaq64.exe`"
                                                {
                                                    wBuf[-1] = L'"';
                                                    ++pDelim;
                                                    ...
                                                    *pDelim = wTemp;
                                                    ...
                                                    pDelim += wAppend[4] == L'6' ? 13 : 11;        //"idaq64.exe' '"        //"idaq.exe' '"
