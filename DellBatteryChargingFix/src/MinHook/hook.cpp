...
static MH_STATUS EnableHookLL(UINT pos, BOOL enable)
{
    ...
    {
        if (pHook->patchAbove)
            memcpyFix(pPatchTarget, pHook->backup, sizeof(JMP_REL) + sizeof(JMP_REL_SHORT));
        else
            memcpyFix(pPatchTarget, pHook->backup, sizeof(JMP_REL));
    }

    VirtualProtect(pPatchTarget, patchSize, oldProtect, &oldProtect);

    // Just-in-case measure.
    FlushInstructionCache(g_hCurrentProc, pPatchTarget, patchSize);
                ...
                if (pBuffer != NULL)
                {
                    ...
                    if (CreateTrampolineFunction(&ct))
                    {
                        ...
                        if (pHook != NULL)
                        {
                            ...
                            pHook->nIP         = ct.nIP;
                            memcpyFix(pHook->oldIPs, ct.oldIPs, ARRAYSIZE(ct.oldIPs));
                            memcpyFix(pHook->newIPs, ct.newIPs, ARRAYSIZE(ct.newIPs));
                            ...
                            {
                                memcpyFix(pHook->backup, pTarget, sizeof(JMP_REL));
                            }
